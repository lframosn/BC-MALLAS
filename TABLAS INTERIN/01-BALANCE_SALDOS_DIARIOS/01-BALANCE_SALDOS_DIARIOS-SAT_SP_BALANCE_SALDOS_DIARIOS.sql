...
BEGIN
  SELECT IDENTIFICADOR_EJECUCION
        ...
        ,FECHA_BALMASALTO  
		,IMP_LIN_DEUDA
   FROM #BALANCE_SALDOS_DIARIOS WHERE 1 = 2
  return 0
END
...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
      SELECT IDENTIFICADOR_EJECUCION
        ...
        ,FECHA_BALMASALTO  
		,IMP_LIN_DEUDA
    FROM SAT.TB_BALANCE_SALDOS_DIARIOS WHERE 1 = 2
      RETURN 0
    END
  END
  IF @OPERACION = 'C'
  BEGIN
     SELECT IDENTIFICADOR_EJECUCION
        ...
        ,FECHA_BALMASALTO  
		,IMP_LIN_DEUDA
    FROM SAT.BALANCE_SALDOS_DIARIOS WHERE 1 = 2
     RETURN 0
  END
END -- 'C'
...
---- actualizacion del saldo de las lineas del cliente
SELECT A.CUENTA
     , SUM(ISNULL(FIN.IMPTOTAL,0) - ISNULL(FIN.IMPINTTOTAL,0) - ISNULL(FIN.IMPAMORT,0)) AS IMP_LIN_DEUDA
  INTO #IMP_LIN_DEUDA
  FROM SAT.TBFINCUO FIN
     , SAT.ATINTCAB ATFIN
     , #BALANCE_SALDOS_DIARIOS A
 WHERE ATFIN.IDENTIFICADOR_EJECUCION = FIN.IDENTIFICADOR_EJECUCION
   AND ATFIN.FECENVIO = A.FECENVIO
   AND FIN.CUENTA = A.CUENTA
GROUP BY A.CUENTA

UPDATE #BALANCE_SALDOS_DIARIOS
   SET IMP_LIN_DEUDA  = I.IMP_LIN_DEUDA
  FROM #BALANCE_SALDOS_DIARIOS A
     , #IMP_LIN_DEUDA I
 WHERE I.CUENTA = A.CUENTA
...
BEGIN
  ...
  IF @FECHA_INI = @FECHA_FIN -- SOLO SE ACTUALIZA LA ULTIMA CORRIDA SI LAS FECHAS SON IGUALES
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM CNF.PARAMETROS WHERE PARAMETRO = 'INTERIN_ULTFEC_BALANCE_SALDOS_DIARIOS')
    BEGIN
      INSERT CNF.PARAMETROS (PARAMETRO, VALOR)
      SELECT 'INTERIN_ULTFEC_BALANCE_SALDOS_DIARIOS',CONVERT(VARCHAR,@FECHA_INI,111)
    END
    ELSE
    BEGIN
      UPDATE CNF.PARAMETROS
         SET VALOR = CONVERT(VARCHAR,@FECHA_INI,111)
       WHERE PARAMETRO = 'INTERIN_ULTFEC_BALANCE_SALDOS_DIARIOS'
    END
  END
END -- OPERACION = 'G'
return 0
