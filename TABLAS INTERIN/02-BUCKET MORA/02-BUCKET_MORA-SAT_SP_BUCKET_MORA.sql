...
BEGIN
  SELECT FECENVIO, IDENTIFICADOR_EJECUCION, CUENTA, DIASMORA, BUCKET_MORA, 
           ...
           MORA4, MORA5, MORA6 FROM #BUCKET_MORA WHERE 1 = 2
  return 0
END
  ...
  BEGIN
    ...
    BEGIN
      SELECT FECENVIO, IDENTIFICADOR_EJECUCION, CUENTA, DIASMORA, BUCKET_MORA, 
           MONTO_MORAINICIAL, MORA1, MORA2, MORA3,
           MORA4, MORA5, MORA6
      FROM SAT.TB_BUCKET_MORA WHERE 1 = 2
      RETURN 0
    END
  END
  IF @OPERACION = 'C'
...
BEGIN
  ...
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM CNF.PARAMETROS WHERE PARAMETRO = 'INTERIN_ULTFEC_BUCKET_MORA')
    BEGIN
      INSERT CNF.PARAMETROS (PARAMETRO, VALOR)
      SELECT 'INTERIN_ULTFEC_BUCKET_MORA',CONVERT(VARCHAR,@FECHA_INI,111)
    END
    ELSE
    BEGIN
      UPDATE CNF.PARAMETROS
         SET VALOR = CONVERT(VARCHAR,@FECHA_INI,111)
       WHERE PARAMETRO = 'INTERIN_ULTFEC_BUCKET_MORA'
    END
  END
END -- OPERACION = 'G'
