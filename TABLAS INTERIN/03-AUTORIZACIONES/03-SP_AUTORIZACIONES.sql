...
BEGIN
  ...
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM CNF.PARAMETROS WHERE PARAMETRO = 'INTERIN_ULTFEC_AUTORIZACIONES')
    BEGIN
      INSERT CNF.PARAMETROS (PARAMETRO, VALOR)
      SELECT 'INTERIN_ULTFEC_AUTORIZACIONES',CONVERT(VARCHAR,@FECHA_INI,111)
    END
    ELSE
    BEGIN
      UPDATE CNF.PARAMETROS
         SET VALOR = CONVERT(VARCHAR,@FECHA_INI,111)
       WHERE PARAMETRO = 'INTERIN_ULTFEC_AUTORIZACIONES'
    END
  END
END -- OPERACION = 'G'
